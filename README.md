...
GaussDBOM 是一个面向运维问题处理的流程平台，覆盖提单、审核、分析、闭环到关闭的完整生命周期，支持排班分单、角色路由、字段联动与基础权限控制。
...
- 运维单八阶段流程（HCS提单 -> 问题审核 -> 运维分析 -> 开发分析 -> 开发闭环 -> 运维闭环 -> 问题审核关闭 -> 问题单关闭）
- 阶段字段动态渲染（文本/富文本/下拉）与必填校验
- 条件联动字段（按字段值显示/必填）
- 工单列表多视图（我的待处理/我创建/我协助的/全部）与筛选
- 自动分单（仅在进入运维人员分析阶段触发：工作日轮值、晚间值班、请假跳过）
- 按提单人角色的分单路由（可后台配置）
- 值班管理月历展示；仅管理员可编辑排班
...
- 前台流程按“阶段处理权限 + 当前处理人”控制（弱化 `ticket:*` 权限点）
- 统计看板权限：登录用户可查看
- 非 `staff/superuser` 用户需绑定角色后获得对应阶段处理能力
- 运维单处理类操作（编辑/推进/转派/回退）仅允许“当前处理人”执行
- 问题审核/分析/闭环阶段按“处理方式”分支流转（与流程字段文档一致）
